...
public class ScrapeManager {
    ...
    private final DevworkScraper scraper = new DevworkScraper();
        ...
        // ExecutorService executor = Executors.newFixedThreadPool(Math.min(urls.size(), 4));
    ExecutorService executor = Executors.newFixedThreadPool(Math.min(urls.size(), 4));
        ...
        for (int i = 0; i < submitted; i++) {
            try {
                ...
                if (result != null) {
                    if (onScraped != null) {
                        try { onScraped.accept(result); } catch (Exception ex) { System.err.println("[ScrapeManager] onScraped handler failed: " + ex.getMessage()); }
                    }
                    results.add(result);
                }
